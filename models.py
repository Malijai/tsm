...
class Tribunal(models.Model):
    ...
    province = models.CharField(max_length=250,verbose_name="1a) Province où se situe ce programme")
    adresse = models.TextField(verbose_name="Adresse complète du programme")
    ...
    tsmfile = models.FileField(upload_to='DocsReferences', verbose_name="4a) Rapport annuel de ce programme", blank=True, null=True)
    ...
    etapejudiciaire = models.ManyToManyField(Judiciaires, verbose_name="8) À quelle étape du processus judiciaire les participants sont-ils référés au programme?")
    reference = models.ManyToManyField(References, verbose_name="9) De qui proviennent les références de vos participants à ce programme?")
    prerequis = models.ManyToManyField(Prerequis, verbose_name="10) Quels sont les critères d’admissibilité?")
    age = models.ManyToManyField(Ages, verbose_name="10+) Quels sont les critères d'age?")
    diagnostic = models.ManyToManyField(Diagnostics, verbose_name="10+) Diagnostics ou antécédents psychiatriques admis")
    typedelits = models.ManyToManyField(Delits, verbose_name="10+) Types de délits ou d'accusations acceptés")
    prerequistexte = models.TextField(verbose_name="10 suite) Détails", blank=True, null=True)
    exclusions = models.ManyToManyField(Exclus, verbose_name="11) Quels sont les critères d’exclusion?")
    decision = models.ManyToManyField(Decisions, verbose_name="12) Qui a un pouvoir décisionnel sur l’admission au programme")
    ...
    traitementtype = models.ManyToManyField(Typetraitements, verbose_name="14) Quels sont les services offerts aux participants?")
    ...
    condition = models.ManyToManyField(Conditions, verbose_name="15) Quelles sont les conditions à respecter une fois dans le programme?")
    ...
    echec = models.ManyToManyField(Echecs, verbose_name="17) Quelles conséquences s’appliquent en cas de non-respect des conditions?")
    echectexte = models.TextField(verbose_name="17a) Non respects détails", blank=True, null=True)
    succes = models.ManyToManyField(Succes, verbose_name="18) Qu’arrive-t-il lorsque le programme est complété avec succès?")
    ...
    partenariat = models.ManyToManyField(Partenaires, verbose_name="22) Avez-vous établi des partenariats avec d’autres services ou organisations?")
    partenariattexte = models.TextField(verbose_name="22a) Si oui : lesquels?", blank=True, null=True)
    reunionfreq = models.ForeignKey(Reunions, verbose_name="P2- Y a-t-il des réunions/discussions d’équipe? Si oui, à quelle fréquence?", on_delete=models.DO_NOTHING)
    reunionpresence = models.TextField(verbose_name="P3- Qui est présent à ces rencontres?", blank=True, null=True)
    limiteparticipants = models.ForeignKey(Limites, verbose_name="S1- Avez-vous une limite de participants?", on_delete=models.DO_NOTHING)
    ...
    communication = models.TextField(verbose_name="3) Avec quels autres programmes de tribunaux spécialisés en santé mentale dans la province (PAJ-SM, IMPAC, PAJIC, par exemple) êtes vous en communication et à quelle fréquence?", blank=True, null=True)
...
class Equipe(models.Model):
    ...
    profession = models.ForeignKey(Professionnels, on_delete=models.DO_NOTHING)
